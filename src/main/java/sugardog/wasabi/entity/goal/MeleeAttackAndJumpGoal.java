...
public class MeleeAttackAndJumpGoal extends Goal {
	protected final TenguIllagerEntity mob;
	...
	public MeleeAttackAndJumpGoal(TenguIllagerEntity p_25552_, double p_25553_, boolean p_25554_) {
		this.mob = p_25552_;
	...
	public void tick() {
		...
		if ((this.followingTargetEvenIfNotSeen || this.mob.getSensing().hasLineOfSight(livingentity)) && this.ticksUntilNextPathRecalculation <= 0 && (this.pathedTargetX == 0.0D && this.pathedTargetY == 0.0D && this.pathedTargetZ == 0.0D || livingentity.distanceToSqr(this.pathedTargetX, this.pathedTargetY, this.pathedTargetZ) >= 1.0D || this.mob.getRandom().nextFloat() < 0.05F)) {
			...
			if (!this.mob.getNavigation().moveTo(livingentity, this.speedModifier)) {
				...
				if (this.mob.distanceToSqr(livingentity) < 64) {
					this.mob.longJumpTriggerByTarget(livingentity);
				}
			}
		}
